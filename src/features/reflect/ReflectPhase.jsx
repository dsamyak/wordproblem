...
const TAKEAWAYS = [
  { icon: '➕', text: 'Clue words like "altogether", "in all" and "more" tell us to ADD.' },
  { icon: '🔵', text: 'A number bond shows two parts joining to make one whole.' },
  { icon: '📊', text: 'A bar model shows the same thing: Part + Part = Whole.' },
  { icon: '🐸', text: 'Counting on with a number line is another way of adding.' },
];

const CONFIDENCE_OPTIONS = [
  { emoji: '🌟', text: "I'm great at solving word problems!" },
  { emoji: '👍', text: 'I can solve most word problems!' },
...
function ReflectQuestion({ question, onAnswer, answered, playNarration }) {
  const [sel, setSel] = useState(null);
  const narratedIdRef = useRef(null);

  useEffect(() => {
    if (!playNarration || !question?.id) return;
    if (narratedIdRef.current === question.id) return;
    narratedIdRef.current = question.id;
    playNarration([{ text: spokenSafe(question.questionText), style: 'question' }]);
  }, [question?.id, question?.questionText, playNarration]);
  ...
  useNumberKeySelect(question.options.length, (idx) => pick(question.options[idx]), !answered && !sel);
    ...
    <div>
      ...
      <div className="options-grid">
        {question.options.map((opt, idx) => {
          ...
          return (
            <button key={opt} className={cls} onClick={() => pick(opt)} style={{ textAlign: 'center' }}>
              <span className="option-key-hint">{idx + 1}</span> {opt}{question.unit ? ` ${question.unit}` : ''}
            </button>
          );
        })}
      </div>
...
export default function ReflectPhase({ onHome, onRestart, playNarration }) {
        ...
        <div style={{ width: '100%', maxWidth: 560 }}>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={qIndex} className="reflect-card glass-card"
              ...
              <ReflectQuestion question={q} onAnswer={handleAnswer} answered={answers.length > qIndex} playNarration={playNarration} />
            </motion.div>
          </AnimatePresence>
        </div>
      </div>
    ...
    return (
      <div className="reflect-screen" style={{ justifyContent: 'center' }}>
        <div className="reflect-card glass-card" style={{ width: '100%', maxWidth: 520, textAlign: 'center' }}>
          ...
          <p style={{ fontWeight: 700, marginBottom: 16 }}>How confident do you feel about solving addition word problems?</p>
      ...
      <div style={{ width: '100%', maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 14, paddingBottom: 16 }}>
        ...
        <div className="reflect-card glass-card" style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--color-text-muted)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.9rem', marginTop: 6 }}>
            {pct >= 80 ? "Outstanding! You're an Addition Ace! 🏆" : pct >= 60 ? "Well done! Keep practising! 💪" : "Review the key facts below 📚"}
          </p>
        </div>
        ...
        <div className="conversion-fact-box">
          <span className="rate-value">Part + Part = Whole</span>
          <div style={{ fontSize: '0.85rem', marginTop: 4, color: 'rgba(255,255,255,0.65)' }}>
            "Altogether", "in all" and "more" all mean: add the parts together
          </div>
        </div>
        ...
        <div className="reflect-badge glass-card">
          ...
          <div>
            ...
            <div className="reflect-badge-sub">Word Problems Using Addition</div>
          </div>
        </div>
